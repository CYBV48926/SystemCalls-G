...
typedef struct user_proc
{
    ...
    TList             children;
    int              (*startFunc) (void*); // function where process begins
	int               pid;// process id
...
int sys_spawn(char* name, int (*startFunc)(void*), char* arg, int stackSize, int priority);
...
int MessagingEntryPoint(char* arg)
{
    ...
    pid = sys_spawn("SystemCalls", SystemCallsEntryPoint, NULL, THREADS_MIN_STACK_SIZE * 4, 3); // create the SystemCalls process, which will execute the SystemCallsEntryPoint function. 
    
	sys_wait(&status); // wait for the SystemCalls process to finish and get its exit status

	// Coded this out due to output result in not an error and inconsistent with test case output "conspole_output(FALSE, "MessagingEntryPoint(): join returned pid = %d, status = %d\n", pid, status);" 
...
static int launchUserProcess(char* pArg)
{
    ...
    {
		resultCode = pProc->startFunc((void*)pProc->arg); // call the startFunc for this process, passing in the arg for this process. Store the return value as the result code for this process.
    }
    else
    {
    ...
    exit(resultCode); // call exit with the result code from the startFunc. This will terminate the process and return the result code to any parent process that is waiting for this process to finish.
...
int k_semv(int sem_id) // V operation on the semaphore with the given sem_id
{
    ...
    for (i = 0; i < MAXSEMS; i++)
    {
		if (semTable[i].sem_id == sem_id) // if we found the semaphore with the given sem_id
        {
			semTable[i].count++; // increment the count to indicate we are releasing one unit of the semaphore
            result = 0;
            break;
        }
    }
...
{
    int pid = k_getpid();
    UserProcess* pProc = findProcByPid(pid);
    if (pProc != NULL)
    {
        pProc->status = STATUS_QUIT;
        pProc->pid    = 0;
    }
    k_exit(resultCode);
}
...
static void spawn_syscall_handler(system_call_arguments_t* args)
{
	int (*startFunc)(void*) = (int (*)(void*)) args->arguments[0]; // cast the first argument to a function pointer with the correct signature
...
{
    int id = (int)(uintptr_t)deviceId; // cast the deviceId to an int for comparison. We are using the deviceId to identify that this interrupt is for a system call
